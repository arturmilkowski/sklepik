        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('backend.admins.products.types.store', $product) }}" method="POST" enctype="multipart/form-data" novalidate>
                        @csrf
                        <x-form-group>
                        ...
                        <x-form-group>
                            ...
                            <x-text-input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Pole obowiązkowe" maxlength="40" required />
                            <x-input-error :messages="$errors->get('name')" />
                        </x-form-group>
                        <x-form-group>
                            <x-input-label for="price">Cena</x-input-label>
                            <x-text-input type="number" id="price" name="price" value="{{ old('price') }}" min="1" max="9999" placeholder="Pole obowiązkowe" required />
                            <x-input-error :messages="$errors->get('price')" />
                        </x-form-group>
                        <x-form-group>
                            <x-input-label for="promo_price">Cena promocyjna</x-input-label>
                            <x-text-input type="number" id="promo_price" name="promo_price" value="{{ old('promo_price') }}" min="0" max="9999" placeholder="Pole nieobowiązkowe" />
                            <x-input-error :messages="$errors->get('promo_price')" />
                        </x-form-group>
                        <x-form-group>
                            <x-input-label for="quantity">Ilość</x-input-label>
                            <x-text-input type="number" id="quantity" name="quantity" value="{{ old('quantity') }}" min="0" max="99" placeholder="Pole obowiązkowe" />
